...
/// stretch doesn't refire on every frame; `PlayerDamage` is inherently event-based already (one hit, one
/// candidate firing). Every rule additionally enforces its own cooldown (see `TryFire`), with a 2-second
/// floor beneath whatever the Sub configured, defensively - the same clamp-not-reject posture every other
/// numeric input in this plugin uses.
public sealed class ToyTriggerEvaluator : IDisposable
{
    ...
    /// Set from the ECommons ActionEffect hook callback, cleared on the next `OnFrameworkUpdate` tick.
    ...
    private volatile bool tookPlayerDamage;
    ...
    /// collar/toy-control "Hit-by-player-action trigger fires": filters for a damage-classified effect
    /// aimed at the local player, sourced from an object of kind Pc (a real player character) - an NPC's
    /// damage never sets this flag, per that requirement's own Non-Goals note.
    ...
    {
        if (type != ActionEffectType.Damage) return;
    ...
    {
        var damagedThisTick = tookPlayerDamage;
        tookPlayerDamage = false;
        ...
        foreach (var rule in config.ToyTriggerRules)
        {
            ...
            switch (rule.Kind)
            {
                ...
                case ToyTriggerKind.PlayerDamage:
                    if (damagedThisTick) TryFire(rule);
                    break;
    ...
    private void TryFire(ToyTriggerRule rule)
    {
        ...
        var fired = rule.PatternName is { Length: > 0 } patternName
            ...
            : toyControl.ForceApplyVibrate(rule.IntensityPercent ?? 50, ToyDuration.Unspecified);
